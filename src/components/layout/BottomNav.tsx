...
import { Home, Search, MessageCircle, Users, Plus } from 'lucide-react'
    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-100 safe-area-pb"
      ...
      <div className="flex items-center justify-around h-16 px-2">
        {tabs.map(({ href, icon: Icon, label, center }) => {
          ...
          const targetHref = (href === '/dashboard' || href === '/vendre') && !user ? '/login' : href
            ...
            return (
              <Link key={href} href={targetHref}
                ...
                <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-transform active:scale-95
                  ${isActive ? 'bg-[#166534] scale-105' : 'bg-[#166534]'}`}
          ...
          return (
            <Link key={href} href={targetHref}
              ...
              <div className={`w-10 h-8 flex items-center justify-center rounded-xl transition-all
                ${isActive ? 'bg-[#166534]/10' : ''}`}>
                <Icon size={22} strokeWidth={1.8}
                  className={isActive ? 'text-[#166534]' : 'text-gray-400'} />
              </div>
